...
ap = argparse.ArgumentParser() #TAKES IN THE VIDEO(ACCEPTING VIDEO FILE)
...
upper = {'red':(186,255,255), 'green':(86,255,255), 'blue':(117,255,255), 'yellow':(54,255,255), 'orange':(20,255,255)} #COLOR DICTIONARY
...
if not args.get("video", False):
    camera = cv2.VideoCapture(0)    #NO VIDEO, OPEN CAMERA
  ...
  #  frame = imutils.resize(frame, width=600)
    ...
    for key, value in upper.items():
        ...
        if len(cnts) > 0:
            ...
            ((x, y), radius) = cv2.minEnclosingCircle(c) #Detecting Circle
            M = cv2.moments(c)
            ...
            if radius > 0.5:
                ...
                cv2.circle(frame, (int(x), int(y)), int(radius), colors[key], 2)        #Making the circle
